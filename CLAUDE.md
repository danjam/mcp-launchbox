...
- `check_library` includes `nearMisses` (up to 5 candidates) when `matches` is empty; fuzzy nearMisses carry confidence 0.40–0.84, while a nearMiss with `shorterTitle: true` (no confidence field) means a shorter title exists — search the shorter title to confirm ownership
